...
function replaceInFile(filePath) {
  ...
  // 9. Add font-mono tabular-nums to numeric displays (simple heuristic for $ or digits inside braces)
  const numberRegex = /\{\s*\$?\d[\d.,]*\s*\}/g;
  let match;
  while ((match = numberRegex.exec(content)) !== null) {
    // locate preceding className attribute within the same element line
