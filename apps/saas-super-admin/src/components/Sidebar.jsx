      ...
      <div className={cn(
        ...
        {!isCollapsed ? (
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-sm tracking-tighter text-zinc-900 dark:text-white uppercase">SaaS Admin</span>
          </div>
        ) : (
          <div className="w-8 h-8 bg-amber-500 text-zinc-950 rounded-lg flex items-center justify-center font-black text-lg">S</div>
        )}
      ...
      <div className="flex-1 overflow-y-auto px-2 py-4 space-y-1">
        {navGroups.map((group) => (
          <div key={group.label} className="mb-4">
            {!isCollapsed && (
              <p className="px-4 text-[9px] font-black text-zinc-400 dark:text-zinc-600 uppercase tracking-widest mb-1 mt-2">{group.label}</p>
            )}
        ...
        {!isCollapsed ? (
          <div className="flex items-center gap-4 p-2 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
             ...
             <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-zinc-900 dark:text-white truncate">{user?.name || 'Administrator'}</p>
                <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase font-mono tracking-wider">SuperAdmin</p>
             </div>
             <Button 
               variant="ghost" 
               size="icon" 
