...
export default function MainLayout() {
  ...
  const getPageTitle = () => {
    ...
    if (path === '/superadmin') return 'SaaS Tenants & Subscriptions';
    if (path === '/command-center') return 'Command Center';
    if (path === '/activity-log') return 'System Activity Logs';
  ...
  useEffect(() => {
    ...
    document.title = `${title} | SaaS SuperAdmin`;
  }, [location.pathname]);
    ...
    <div className="flex min-h-screen bg-background font-sans text-foreground">
      ...
      <div className="flex flex-col flex-1 min-h-screen min-w-0 transition-all duration-300">
        ...
        <header className="h-16 flex items-center justify-between px-6 sticky top-0 z-[60] bg-background/80 backdrop-blur-md border-b border-border/50">
          <div className="flex items-center gap-4">
            ...
            <Sheet>
              ...
              <SheetContent side="left" className="p-0 w-64 bg-zinc-950">
                ...
                <SheetDescription className="sr-only">Akses cepat ke seluruh modul SaaS Admin</SheetDescription>
                <Sidebar
                   isOpen={true}
                   onClose={() => {}}
                   isCollapsed={false}
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-amber-500/5 rounded-sm border border-amber-500/20 text-[9px] text-amber-500 font-black uppercase tracking-widest">
        ...
        <footer className="px-6 py-2 border-t text-[10px] text-muted-foreground/40 flex items-center justify-between">
          <span className="flex items-center gap-1.5"><Coffee size={10} /> KEN ENTERPRISE v5.0</span>
